...
impl PreRun {
    ...
    pub async fn setup(
        &self,
        sender: Option<mpsc::Sender<AppUpdate>>,
        sql: Option<String>,
    ) -> Result<String, String> {
        // Step 1: Submit SQL task
        if let Some(sender) = &sender {
            let _ = sender
                .send(AppUpdate::SetupProgress(
        ...
        let results = self
            .wait_for_results(&session_handle, &operation_handle)
            .await?;

        Ok(format!(
            "Setup completed successfully. Results:\n{}",
            results
        ))
    }
    ...
    async fn create_session(&self) -> Result<String, String> {
        ...
        loop {
            match client
                .post(format!("{}/v1/sessions", self.api_endpoint))
                ...
                .await
            {
                    ...
                    match serde_json::from_str::<serde_json::Value>(&text) {
                        ...
                        Err(e) => {
                            ...
                            if attempts == max_attempts {
                                return Err(format!(
                                    "Failed to parse session response after {} attempts: {}",
                                    max_attempts, e
                                ));
                            }
                        }
                ...
                Err(e) => {
                    ...
                    if attempts == max_attempts {
                        return Err(format!(
                            "Failed to create session after {} attempts: {}",
                            max_attempts, e
                        ));
                    }
                }
            ...
            if attempts >= max_attempts {
                return Err(format!(
                    "Failed to create valid session after {} attempts",
                    max_attempts
                ));
            }
    ...
    async fn create_catalog(&self, session_handle: &str) -> Result<(), String> {
        ...
        let response = client
            .post(format!(
                ...
                self.api_endpoint, session_handle
            ))
        ...
        println!("👉 Create catalog response {:#}", json);
        ...
        loop {
                ...
                .await
            {
                Ok(response) => match response.json::<serde_json::Value>().await {
                    ...
                    Err(e) => {
                        if retries >= max_retries {
                            return Err(format!(
                                "Failed to parse query response after {} retries: {}",
                                max_retries, e
                            ));
                        }
                    }
                ...
                Err(e) => {
                    if retries >= max_retries {
                        return Err(format!(
                            "Failed to submit query after {} retries: {}",
                            max_retries, e
                        ));
                    }
                }
    ...
    async fn wait_for_results(
        ...
        let mut next_uri = format!(
            "/v1/sessions/{}/operations/{}/result/0",
        ...
        loop {
            if retries >= max_retries {
                return Err(format!(
                    "Timeout waiting for results after {} retries",
                    max_retries
                ));
            }

            let response = client
                .get(format!("{}{}", self.api_endpoint, next_uri))
                .header("Authorization", self.get_auth_header())
                .send()
                .await
            ...
            match json.get("resultType").and_then(|rt| rt.as_str()) {
                ...
                Some("PAYLOAD") => {
                    if let Some(data) = json.get("results") {
                            ...
                            .map_or(false, |arr| !arr.is_empty())
                        {
                            ...
                            // Extract data rows
                            let rows = data.get("data").and_then(|d| d.as_array()).map(|rows| {
                                rows.iter()
                ...
                rt => return Err("Unknown result type".to_string()),
            }
            retries += 1;
